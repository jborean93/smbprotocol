...
class TestDFSReferralRequest(object):
    ...
    def test_parse_message(self):
        ...
        assert actual['request_file_name'].get_value() == '\\\\server\\shares\\share'


class TestDFSReferralResponse(object):

    def test_create_message(self):
        message = DFSReferralResponse()
        message['path_consumed'] = 46
        message['number_of_referrals'] = 0
        message['referral_header_flags'] = DFSReferralHeaderFlags.STORAGE_SERVERS | \
            DFSReferralHeaderFlags.REFERRAL_SERVERS
        message['referral_entries'] = []
        expected = b'.\x00' \
                   b'\x00\x00' \
                   b'\x03\x00\x00\x00'
        actual = message.pack()
    ...
    def test_parse_message(self):
        ...
        actual.unpack(data)
        assert len(actual) == 8
        assert actual['path_consumed'].get_value() == 46
        assert actual['number_of_referrals'].get_value() == 0
        assert actual['referral_header_flags'].get_value() == DFSReferralHeaderFlags.STORAGE_SERVERS | \
            DFSReferralHeaderFlags.REFERRAL_SERVERS
        assert actual['referral_entries'].get_value() == []


class TestDFSReferralEntry(object):

    def test_create_message(self):
        message = DFSReferralEntry()
        message['version_number'] = 4
        message['size'] = 34
        message['server_type'] = DFSServerTypes.ROOT_TARGETS
        message['referral_entry_flags'] = DFSReferralEntryFlags.TARGET_SET_BOUNDARY
        message['time_to_live'] = 300
        message['dfs_path_offset'] = 34
        ...
        expected = b'\x04\x00' \
                   ...
                   b'\\\x00s\x00e\x00r\x00v\x00e\x00r\x00\\\x00s\x00h\x00a\x00r\x00e\x00s\x00' \
                   b'\\\x00s\x00h\x00a\x00r\x00e\x00\x00\x00' \
                   b'\\\x00s\x00e\x00r\x00v\x00e\x00r\x00\\\x00s\x00h\x00a\x00r\x00e\x00s\x00' \
                   b'\\\x00s\x00h\x00a\x00r\x00e\x00\x00\x00' \
                   b'\\\x00s\x00m\x00b\x00s\x00e\x00r\x00v\x00e\x00r\x00\\\x00s\x00h\x00a\x00r\x00e\x00s\x00' \
                   b'\\\x00s\x00h\x00a\x00r\x00e\x00\x00\x00'
        actual = message.pack()
        assert len(message) == 166
        assert actual == expected

    def test_parse_message(self):
        actual = DFSReferralEntry()
        data = b'\x04\x00' \
               b'"\x00' \
               b'\x01\x00' \
               b'\x04\x00' \
        ...
        assert actual['version_number'].get_value() == 4
        assert actual['size'].get_value() == 34
        assert actual['server_type'].get_value() == DFSServerTypes.ROOT_TARGETS
        assert actual['referral_entry_flags'].get_value() == DFSReferralEntryFlags.TARGET_SET_BOUNDARY
        assert actual['time_to_live'].get_value() == 300
        assert actual['dfs_path_offset'].get_value() == 34
        assert actual['dfs_alternate_path_offset'].get_value() == 76
        assert actual['network_address_offset'].get_value() == 118
        assert actual['service_site_guid'].get_value() == uuid.UUID(bytes=b"\xff" * 16)
        assert actual['dfs_path'].get_value() == '\\server\\shares\\share'
        assert actual['dfs_alternate_path'].get_value() == '\\server\\shares\\share'
        assert actual['network_address'].get_value() == '\\smbserver\\shares\\share'
