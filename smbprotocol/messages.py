...
from smbprotocol.constants import Capabilities, Ciphers, CloseFlags, \
    ...
    CreateOptions, CtlCode, Dialects, FileAttributes, FileInformationClass, \
    FileFlags, HashAlgorithms, ImpersonationLevel, IOCTLFlags, \
    NegotiateContextType, NtStatus, QueryDirectoryFlags, ReadFlags, \
    ReadWriteChannel, RequestedOplockLevel, SecurityMode, SessionFlags, \
    ShareAccess, ShareCapabilities, ShareFlags, ShareType, Smb1Flags2, \
    Smb2Flags, TreeFlags, WriteFlags
...
class SMB2QueryDirectoryRequest(Structure):
    ...
    def __init__(self):
        self.fields = OrderedDict([
            ...
            ('file_name_offset', IntField(
                size=2,
                default=lambda s: self._get_file_name_offset(s)
            )),
            ('file_name_length', IntField(
                size=2,
                default=lambda s: len(s['buffer'])
            )),
            ('output_buffer_length', IntField(size=2)),
            ('buffer', BytesField(
    ...
    def _get_file_name_offset(self, structure):
        if len(structure['buffer']) == 0:
            return 0
        else:
